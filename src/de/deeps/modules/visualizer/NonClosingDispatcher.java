 ...
 * players, ...
 * Using a (blocking) audio player it is even possible to synchronize execution
 * of
 * AudioProcessors and sound. This behavior can be used for visualization.
 * 
 * @author Joren Six
 */
public class NonClosingDispatcher implements Runnable {

	/**
	 * Log messages.
	 */
	private static final Logger LOG = Logger
			.getLogger(NonClosingDispatcher.class.getName());
	 ...
	 */
	public NonClosingDispatcher(final TarsosDSPAudioInputStream stream,
	 ...
	 * size - hop size
	 * E.g. if the buffer is 2048 and the hop size is 48 then you get 2000x0 and
	 * 48 filled audio samples
	 * 
	 ...
	 * until the normal amount
	 * of elements are present in the buffer. Otherwise the buffer only contains
	 * the last elements and no zeros.
	...
	public void addAudioProcessor(final AudioProcessor audioProcessor) {
		...
		LOG.fine(
			"Added an audioprocessor to the list of processors: "
	...
	public void removeAudioProcessor(final AudioProcessor audioProcessor) {
		...
		LOG.fine(
			"Remove an audioprocessor to the list of processors: "
					+ audioProcessor.toString());
	}
	...
	public void run() {
		...
		try {
			...
			String message = "Error while reading audio input stream: "
					+ e.getMessage();
			LOG.warning(message);
	...
	private int readNextAudioBlock() throws IOException {
		...
		if (isFirstBuffer && !zeroPadFirstBuffer) {
			...
			// In all other cases read the amount of bytes defined by the step
			// size
		...
		if (!isFirstBuffer
				&& audioFloatBuffer.length == floatOverlap + floatStepSize) {
			System.arraycopy(
				audioFloatBuffer,
				floatStepSize,
				audioFloatBuffer,
				...
				floatOverlap);
			/*
			 * for(int i = floatStepSize ; i < floatStepSize+floatOverlap ;
			 * i++){
		...
		// unless the stream is closed (stopped is true) or no bytes could be
		...
		if (endOfStream) {
			// Could not read a full buffer from the stream, there are two
			// options:
			if (zeroPadLastBuffer) {
				// Make sure the last buffer has the same length as all other
				// buffers and pad with zeros
				for (int i = offsetInBytes
						+ totalBytesRead; i < audioByteBuffer.length; i++) {
					audioByteBuffer[i] = 0;
				}
				...
				audioFloatBuffer = new float[offsetInSamples
						+ totalBytesRead / format.getFrameSize()];
				converter.toFloatArray(
					audioByteBuffer,
					offsetInBytes,
					audioFloatBuffer,
			...
			// If the end of the stream has not been reached and the number of
			// bytes read is not the
